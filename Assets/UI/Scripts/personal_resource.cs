...
public class Personal_resource : MonoBehaviour, IDamageable, IEnemyAttackable
{
    ...
    public int Wooden
    {
        get { return bag.resources[(int)PlayerBag.ResourceKind.Wooden]; }
        set { bag.resources[(int)PlayerBag.ResourceKind.Wooden] = value; }
    }
    public int Steel
    {
        get { return bag.resources[(int)PlayerBag.ResourceKind.Steel]; }
        set { bag.resources[(int)PlayerBag.ResourceKind.Steel] = value; }
    }
    public int Metal
    {
        get { return bag.resources[(int)PlayerBag.ResourceKind.Metal]; }
        set { bag.resources[(int)PlayerBag.ResourceKind.Metal] = value; }
    }

    private static readonly float maxhp = 100.0f;
    private static float nowhp = 100.0f;

    public static float MaxHP
    {
        get { return maxhp * TechTreeUnlock.playerHP; }
    }
    public static float NowHP
    {
        get { return nowhp * TechTreeUnlock.playerHP; }
    ...
    public void Damage(
        ...
        float stiffenTime = 0.15f
    )
    {
        bool isEquipedArmor = ItemOwnManager.ownWeapon[Kind.Armor][
            ObjectPoolManager.instance[Kind.Armor].weaponIndex
        ];
        ...
        NowHP =
            ...
            - (
                damage
                / TechTreeUnlock.playerHP
                * TechTreeUnlock.playerReceiveDamage
                * (Weapons.Weapon.isAttacking ? TechTreeUnlock.duringAttackingReceiveDamage : 1)
                * (NowHP / MaxHP <= 0.1f ? TechTreeUnlock.lowHpReceiveDamage : 1)
                * (continousHitStack > 0 ? TechTreeUnlock.continuousReceiveDamage : 1)
                * (
                    Vector2.Dot(rb.linearVelocity.normalized, knockBackDirection.normalized)
                    >= Mathf.Cos(45 * Mathf.Deg2Rad)
                        ? TechTreeUnlock.playerReceiveDamageOnBack
                        : 1
                )
                * (
                    type == AttackType.Close
                        ? TechTreeUnlock.closeAttackReceiveDamage
                        : (
                            type == AttackType.Explosion
                                ? TechTreeUnlock.explosiveAttackReceiveDamage
                                : 1
                        )
                )
                / (
                    1
                    + TechTreeUnlock.duringNoDamageIncreaseDefence
                        * Mathf.Clamp((int)(NoDamageTimer / 5), 0, TechTreeUnlock.S29MAXOVERWRAP)
                )
            )
                * (
                    isEquipedArmor
                        ? Armor
                            .armorStats[ObjectPoolManager.instance[Kind.Armor].weaponIndex]
                            .defence
                        : 1
                );

        if (!TechTreeUnlock.haveIgnoreKnockBack)
        ...
        if (damage > 0)
        {
            ...
            StartCoroutine(
                WaitAction.wait(
                    stiffenTime * TechTreeUnlock.stiffenTime,
                    () =>
                    {
                        isStiffen = false;
                    }
