...
public enum WorkbenchInputTaskType
{
   Default,
   rifle,
   riflebullet
}
public class WorkbenchInputTaskTuto : MonoBehaviour
{
    [SerializeField] private WorkbenchInputTaskType currentTaskType = WorkbenchInputTaskType.Default;
    ...
    private void Update()
    {
        if (spendTime != -1)
        {
            if (!isComplete)
            {
                ...
                timeCheck.text = $"대기 순서 : {taskQueue.ToList().IndexOf(timeCheck)}\n남은 시간 : {(int)(WorkingManager.remainingTimeTuto[this] / 60f)}분 {(int)(WorkingManager.remainingTimeTuto[this] % 60)}초";
            }
            else
            {
    ...
    public void StartTask()
    {
            ...
            totalMetal >= reqMetal && Resource.public_watt >= reqWatt)
        {
            ...
            WorkingManager.remainingTimeTuto[this] = spendTime;
            taskBar.fillAmount = 0;
            taskBar.transform.parent.gameObject.SetActive(true);
            if(Tutorial.instance != null && currentTaskType == WorkbenchInputTaskType.rifle && Tutorial.instance.nowpage == 7)
            {
                Tutorial.instance.nextpage();
            }
            if(Tutorial.instance != null && currentTaskType == WorkbenchInputTaskType.riflebullet && Tutorial.instance.nowpage == 8)
            {
                Tutorial.instance.nextpage();
            }
        }
